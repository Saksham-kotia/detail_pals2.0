    ...
    <div className="p-8 text-white flex gap-6 h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between mb-6 shrink-0">
          <div>
            <h1 className="text-2xl font-light tracking-widest uppercase">Bookings</h1>
            <p className="text-white/30 text-sm mt-1">{count} total</p>
          </div>
          <button
            onClick={refresh}
      ...
      {/* Right: detail panel */}
      {selected && (
        <aside className="w-80 shrink-0 border border-white/10 rounded-xl overflow-y-auto p-6">
          ...
          <Section title="Services">
            {(selected.services as Array<{name:string;price:number}>).map((s, i) => (
              <div key={i} className="flex justify-between text-sm py-1">
                <span className="text-white/70">{s.name}</span>
                <span className="text-white/50">{formatCurrency(s.price)}</span>
              </div>
            ))}
            {(selected.addons as Array<{name:string;price:number}>).map((a, i) => (
              <div key={i} className="flex justify-between text-sm py-1">
          ...
          <div className="mt-4">
            <p className="text-[10px] tracking-[2px] text-white/30 uppercase mb-2">Change Status</p>
            <select
              value={selected.status}
              onChange={e => handleStatusChange(selected.id, e.target.value as BookingStatus)}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-white/20 capitalize font-sans"
            >
              <option value="pending" className="bg-[#111]">Pending (Awaited)</option>
              <option value="confirmed" className="bg-[#111]">Accepted (Send Email)</option>
